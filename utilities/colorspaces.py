...
def get_colorspaces(OCIOv):
	...
	display_referred_colorspaces = [
		...
		Colorspace(
			name = 'Display Encoding - Display P3',
			alias = 'p3dsp',
			desc = 'Display encoding - Apple Display P3 with P3 primaries and pure 2.2 power EOTF (NOT the Piecewise OETF as wrongly defined in the spec, since most apple devices use 2.2 power EOTF) \n https://developer.apple.com/documentation/coregraphics/cgcolorspace/displayp3',
			family = 'Display-Referred/Display Encoding',
			encoding = 'sdr-video',
			chr = 'P3D65 - Linear',
			tf = ocio_xform_exp(2.2, inv=True),
		...
		Colorspace(
			name = 'Display Encoding - Rec2100 PQ',
			alias = 'pq',
			desc = 'Display encoding - Rec.2020 Primaries with ST.2084 PQ Transfer Function.',
			family = 'Display-Referred/Display Encoding',
			encoding = 'sdr-video',
			chr = 'Rec2020 - Linear',
			tf = tfuncs.get('eotf_st2084'),
			forward = False,
		),
	]
